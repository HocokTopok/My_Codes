...
int64_t DFS(Data& data, int64_t current_vertex = kStartVertex,
  ...
  for (Edge& edge : data.edges[current_vertex]) {
    ...
    int64_t res_potok = DFS(
        data, edge.vertex_to,
        std::min(delta,
                 edge.capacity));  // Когда добавлю в репу то нужно заменить
                                   // нейминг capacity - это в остаточной сети
    if (res_potok == 0) {
      continue;
    }

    edge.capacity -= res_potok;
    data.edges[edge.vertex_to][edge.reverse_index].capacity += res_potok;
