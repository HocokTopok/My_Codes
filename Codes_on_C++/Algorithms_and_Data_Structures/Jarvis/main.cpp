...
void Jarvis(Data& data) noexcept {
  Dot current_dot = data.start_dot;
  data.AddVertex(current_dot);
  ...
  while (true) {
    ...
    for (const Dot& new_dot : data.dots) {
      if (new_dot == current_dot) {
        continue;
      }

      long long cr =
          Cross(Vector(current_dot, top_dot), Vector(current_dot, new_dot));

      if (cr > 0) {
        top_dot = new_dot;
      } else if (cr == 0) {
        if (CompareDist(current_dot, top_dot, new_dot)) {
          top_dot = new_dot;
        }
      }
    }

    if (top_dot == data.start_dot) {
      break;
    }
