...
Data Input() {
  ...
  Data data(vertex_count + 1);
  for (size_t u_num = 1; u_num <= vertex_count; ++u_num) {
    for (size_t v_num = 1; v_num <= vertex_count; ++v_num) {
      size_t w_num;
      std::cin >> w_num;
      if (u_num != v_num) {
  ...
  for (size_t v_num = 1; v_num <= vertex_count; ++v_num) {
    size_t w_num;
    std::cin >> w_num;
    data.graph[0].push_back({w_num, v_num});
    data.graph[v_num].push_back({w_num, 0});
  }
